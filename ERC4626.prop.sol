...
abstract contract ERC4626Prop is Test {
    ...
    function prop_deposit(address caller, address receiver, uint assets) public {
        ...
        assertApproxEqAbs(newCallerAsset, oldCallerAsset - assets, __delta__, "asset"); // NOTE: this may fail if the caller is a __vault__ in which the asset is stored
    ...
    function prop_withdraw(address caller, address receiver, address owner, uint assets) public {
        ...
        assertApproxEqAbs(newReceiverAsset, oldReceiverAsset + assets, __delta__, "asset"); // NOTE: this may fail if the receiver is a __vault__ in which the asset is stored
    ...
    function assertApproxGeAbs(uint a, uint b, uint maxDelta/*, string memory err*/) internal {
        if (!(a >= b)) {
            uint dt = b - a;
            if (dt > maxDelta) {
    ...
    function assertApproxLeAbs(uint a, uint b, uint maxDelta/*, string memory err*/) internal {
        if (!(a <= b)) {
            uint dt = a - b;
            if (dt > maxDelta) {
              //emit log_named_string   ("Error", err);
